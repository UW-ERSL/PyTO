...
import mat_lib
import bound_cond
from tet_thermal_fea import ThermalFEATet as ttf
...
class TetTransientThermalFEA:
    def __init__(self,
        ...
        for i in range(self.mesh.num_elems):
            elem_stiff =  jnp.asarray(ttf.tet4_stiffness_matrix_thermal(mat_prop, self.mesh.nodes[self.mesh.edofMat[i, :]]))
...
if __name__ == "__main__":
    ...
    transient_solver = TetTransientThermalFEA(mesh = mesh,
                              mat_prop = mat_prop,
                              bc = bc,
                              deltaTime = deltaTime,
