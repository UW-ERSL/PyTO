...
#################################################################
  ...
  def getHPrime(self, E, alpha, dx, dy, dz, nu):
    """
  ...
  See Deng, S. and Suresh, K., 2017. Stress constrained thermo-elastic topology optimization with varying temperature fields via augmented topological sensitivity based level-set. 
    ...
    This function was converted from MATLAB code generated by the Symbolic Math Toolbox version 24.1.
    08-Nov-2025 15:37:43
    ...
    t3 = t2 - 1.0
    t4 = 1.0 / t3
    t5 = (E * alpha * dx * dy * t4) / 1.8e+1
    t6 = (E * alpha * dx * dy * t4) / 3.6e+1
    t7 = (E * alpha * dx * dz * t4) / 1.8e+1
    t8 = (E * alpha * dx * dz * t4) / 3.6e+1
    t9 = (E * alpha * dy * dz * t4) / 1.8e+1
    t10 = (E * alpha * dy * dz * t4) / 3.6e+1
    t13 = (E * alpha * dx * dy * t4) / 7.2e+1
    t16 = (E * alpha * dx * dz * t4) / 7.2e+1
    t19 = (E * alpha * dy * dz * t4) / 7.2e+1
    t11 = -t5
    t12 = -t6
    t14 = -t7
    t15 = -t8
    ...
    # Create the array with all elements
    HPrime_flat = np.array([
    ...
    # Reshape to 24x8 matrix (column-major order like MATLAB)
    # MATLAB's reshape fills column-by-column, so we use Fortran-style ordering
    HPrime = HPrime_flat.reshape((24, 8))
    
    return HPrime
  
  def get_thermoelastic_force(self):
    """
    ...
    for elem in range(self.mesh.num_elems):
      ...
      f_thermal_elem = pseudodensity[elem] * HPrime @ (node_temp - self.thermoElasticReferenceTemperature)
