...
def topopt_levelset(feaMode,
    ...
    def verify_derivative_operator(mesh, HXD):
        """Verify derivative operator on a linear test function."""
        a, b = 2.0, 1.0
        f = a * mesh.elem_centers[:, 0] + b  # Linear in x
        df_dx_numerical = HXD @ f
        df_dx_exact = np.full(mesh.num_elems, a)
        
        error = np.linalg.norm(df_dx_numerical - df_dx_exact) / np.linalg.norm(df_dx_exact)
        print(f"Derivative operator relative error: {error:.2e}")
        assert error < 1e-10, "Derivative operator failed linear function test"
     
    #verify_derivative_operator(fe_solver.mesh, HXD)
    ...
    #fe_solver.plot_elem_field(rho, title="Initial Pseudo Density")
    ...
    #fe_solver.plot_elem_field(lsf, title="Initial Level Set Function",cross_section=('x',0.5))
