...
class TopOptSTLRecovery:
    ...
    def extract_low_density_patches(self,vtu, density_field='density', threshold=0.5):
        """
    ...
    def split_and_filter_patches(self,all_low_density_elems, cells_threshold_percentage=5):
        """
    ...
    def clean_mesh(self,mesh, name="mesh", hole_fraction=0.01, feature_angle=30):
        """
    ...
    def enhanced_smooth(self,mesh, iterations=50, pass_band=0.2):
        """
    ...
    def advanced_repair_meshfix(self,mesh):
        """
    ...
    def post_process_final_mesh(self,mesh):
        """
        ...
        mesh = self.advanced_repair_meshfix(mesh)

        return mesh

    def process_void_patch(self,patch, original_mesh, boundary_threshold=0.01, shrink_factor=0.9):
        """
        ...
        patch = self.enhanced_smooth(patch, iterations=50, pass_band=0.2)
        
        # Clean and repair with light touch
        patch = self.clean_mesh(patch, "void patch", feature_angle=15, hole_fraction=0.01)
        ...
        try:
            patch = self.post_process_final_mesh(patch)
        ...
        try:
            ...
            if delaunay.n_cells == 0:
                ...
                delaunay = self.clean_mesh(delaunay, "delaunay void", hole_fraction=0.01)
                
                # Only apply advanced repair if we have a good mesh
                if delaunay.n_cells > 0:
                    try:
                        delaunay = self.advanced_repair_meshfix(delaunay)
        ...
        try:
            tm_void = self.to_trimesh(delaunay)
            ...
            tm_void = self.adjust_void_for_boundary(tm_void, original_mesh, 
    ...
    def validate_void_patch(self,void_mesh, original_mesh):
        """
    ...
    def scale_void_for_subtraction(self,void_mesh, original_mesh, z_scale_factor=1.2):
        """
    ...
    def is_boundary_void(self,void_mesh, original_mesh, boundary_threshold=0.01):
        """
    ...
    def adjust_void_for_boundary(self,void_mesh, original_mesh, boundary_threshold=0.01, shrink_factor=0.9):
        """
        ...
        if not self.is_boundary_void(void_mesh, original_mesh, boundary_threshold):
    ...
    def apply_boundary_mask(self,original_mesh, final_mesh, boundary_thickness=0.05):
        """
    ...
    def create_stl_from_topopt_result(self,design_domain_stl_filepath=None, topopt_result_vtu_filepath=None, 
                           preserve_boundary=True, boundary_thickness=0.05,
        ...
        Parameters:
        -----------
        design_domain_stl_filepath : str
            Path to input STL file
        topopt_result_vtu_filepath : str
        ...
        try:
            ...
            print(f"Loading STL from: {design_domain_stl_filepath}")
            original = pv.read(design_domain_stl_filepath).triangulate().compute_normals()
            ...
            print(f"Loading VTU from: {topopt_result_vtu_filepath}")
            voxels = pv.read(topopt_result_vtu_filepath)
            ...
            tm_base = self.to_trimesh(original)
            ...
            for i, patch in enumerate(patch_list):
                ...
                tm_void = self.process_void_patch(patch, original, 
                                        boundary_threshold=boundary_threshold,
                                        shrink_factor=shrink_factor)
                ...
                tm_void = self.scale_void_for_subtraction(tm_void, original, z_scale_factor=1.2)
                ...
                if self.visualize:
                    ...
                    if not self.validate_void_patch(tm_void, original):
            ...
            if preserve_boundary:
                ...
                preserved_result = self.apply_boundary_mask(original_clean, pv_result, boundary_thickness)
                
                # Convert to trimesh for export
                preserved_tm = self.to_trimesh(preserved_result)
            ...
            #result_tm = self.to_trimesh(pv_result)
