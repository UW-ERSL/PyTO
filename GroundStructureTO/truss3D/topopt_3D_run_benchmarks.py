...
from topopt_density_mma_gsa_3Dtruss import topopt_mma_gsa
from topopt_density_oc_gsa_3Dtruss import topopt_optimality_criteria_gsa
from topopt_pareto_gsa_3Dtruss import topopt_pareto_gsa	
from trussopt_3D_to_PyTO_pyvista import get_3D_rho_from_2D
...
def runTOMethodOnBenchmarks(optimizationMethod):
	...
	benchmarks_2_5D_problems = [StructuralTOExamples.Mitchell_1, 
						StructuralTOExamples.ShortCantileverTipLoad, StructuralTOExamples.ShortCantileverMidLoad,
						StructuralTOExamples.CantileverTipLoad, StructuralTOExamples.CantileverMidLoad,
						StructuralTOExamples.TwoBar, StructuralTOExamples.MBBB]
	benchmarks_3D_problems = [StructuralTOExamples.EdgeCantilever]
	...
	for to_problem in benchmarks_2_5D_problems:
		...
		if optimizationMethod == TO_METHODS.DENSITYMMA:
			...
			u, history, success,errorMsg,nFEAs = topopt_levelset(fe_solver = fe_solver,
													to_params = to_params)
		elif optimizationMethod == TO_METHODS.GSA_DENSITYMMA:
			x0 = get_3D_rho_from_2D(to_problem, fe_solver.mesh, target_volfrac= to_params.DesiredVolFraction, use_binary_fill = False, b_plot = False)  
			u, history, success,errorMsg,nFEAs = topopt_mma_gsa(fe_solver = fe_solver,
													to_params = to_params, x0 = x0)
		elif optimizationMethod == TO_METHODS.GSA_DENSITYOC:
			x0 = get_3D_rho_from_2D(to_problem, fe_solver.mesh, to_params.DesiredVolFraction, use_binary_fill = False, b_plot = False)  
			u, history, success,errorMsg,nFEAs = topopt_optimality_criteria_gsa(fe_solver = fe_solver,
													to_params = to_params, x0 = x0)
		elif optimizationMethod == TO_METHODS.GSA_PARETO:
			x0 = get_3D_rho_from_2D(to_problem, fe_solver.mesh, to_params.DesiredVolFraction*1.01, use_binary_fill = False, b_plot = False)  
...
if __name__ == "__main__":    
	...
	#optimizationMethods = [TO_METHODS.DENSITYMMA, TO_METHODS.DENSITYOC, TO_METHODS.PARETO, TO_METHODS.GSA_DENSITYMMA, TO_METHODS.GSA_DENSITYOC, TO_METHODS.GSA_PARETO]
	# optimizationMethods = [TO_METHODS.DENSITYMMA, TO_METHODS.GSA_DENSITYMMA]
