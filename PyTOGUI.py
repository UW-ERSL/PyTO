...
class OptimizeTopologyWindow(QtWidgets.QDialog):
    ...
    def apply_topopt_constraints(self, to_params, constraints):
        ...
        if cyclic_symmetry.get('enabled', False):
            angle_text = cyclic_symmetry.get('angle', '90 deg')
            ...
            if match:
                try:
                    ...
                    angle = int(float(match.group()))
                    
                    # Validate angle - must be a divisor of 360 and at least 2
                    if angle > 0 and 360 % angle == 0 and angle >= 2:
                        # Valid angle, set it
                        to_params.ZAxisAngularSymmetry = angle
                        self.log_message(f"Applied Z-axis angular symmetry with angle {angle}°")
                    else:
                        # Invalid angle, disable the constraint
                        self.log_message(f"Warning: Invalid angular symmetry value: {angle}°. Must be a positive divisor of 360 (e.g., 2, 3, 4, 5, 6, 8, 9, 10, 12, 15, etc.)")
                        to_params.ZAxisAngularSymmetry = 0
                except (ValueError, TypeError):
                    self.log_message(f"Warning: Could not parse angular symmetry value from '{angle_text}'. Disabling constraint.")
                    to_params.ZAxisAngularSymmetry = 0
            else:
                # No valid angle found in text
                self.log_message(f"Warning: No valid angle found in '{angle_text}'. Disabling cyclic symmetry constraint.")
                to_params.ZAxisAngularSymmetry = 0
        else:
    ...
    def run_optimization(self, method, volume_fraction, save_intermediate):
        ...
        try:
            ...
            mesh, mat_prop, bc = AnalysisWindow.ProcessDataforSolver(
                existing_mesh=mesh,
                fixed_nodes=fixed_nodes,
                load_data=load_data,
    ...
    def optimization_completed(self, success, error_msg):
        ...
        if success:
            ...
            # Visualize results
            if hasattr(self.parent, 'visualize_optimized_topology'):
                self.parent.visualize_optimized_topology()
